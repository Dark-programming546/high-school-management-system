        ...
        {!loading && stats && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <StatCard icon="🏫" label="Total Classes"   value={stats.totalClasses   ?? 0} color="purple" />
              <StatCard icon="👥" label="Total Students"  value={stats.totalStudents  ?? 0} color="green"  />
              <StatCard icon="👨🏫" label="Total Teachers" value={stats.totalTeachers  ?? 0} color="blue"   />
                  ...
                  <ol className="space-y-2 text-sm">
                    {stats.topStudents.map((entry, i) => (
                      <li key={i} className="flex justify-between items-center">
                        <span>
                          <span className="font-bold text-purple-600 mr-2">#{entry.schoolRank ?? i + 1}</span>
